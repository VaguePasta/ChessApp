...
let oldPos = props.information.Piece[2]
...
watch(props.information, (newPosition) => {
  ...
  if (oldPos === newPosition.Piece[2]) return
  if (props.side === 0) {
    oldLeft.value = ((oldPos % 8) * 12.5).toString() + '%'
    oldTop.value = (Math.floor(oldPos / 8) * 12.5).toString() + '%'
  }
  else {
    oldLeft.value = (87.5 - ((oldPos) % 8) * 12.5).toString() + '%'
    oldTop.value = (87.5 - Math.floor((oldPos) / 8) * 12.5).toString() + '%'
  }
  oldPos = newPosition.Piece[2]
})
</script>
